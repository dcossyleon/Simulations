...
# file name: first number refers to correlation between the two factors,
#            second to allele freq p
#            third to perc variance explained
#            so 2fact2_5_2=> corr between factors =.2,,p=.5,%varexpl=1/2
namefile=as.character("2fact2_5_1")            # nr of items
...
geffect=(perc*1)/(1-perc) #perc is variance explained
...
Y1=matrix(c(1,.2,.2,1),nfac,nfac,byrow=T)            # factor variance=1 (unconditional)
Y2=matrix(c(1,.2,.2,1),nfac,nfac,byrow=T)

# define residuals
# calc residuals such that L^2 + E^2 = 1
E1=diag(1,nvar)
for (i in 1:nvar) {
  E1[i,i]=E1[i,i]-((L1[i,1]^2*Y1[1,1])
                   +(L1[i,2]^2*(Y1[2,2]+(a1*varg1*a1))))
} 

E2=diag(1,nvar)
for (i in 1:nvar) {
  E2[i,i]=E2[i,i]-((L2[i,1]^2*Y2[1,1])
                   +(L2[i,2]^2*(Y2[2,2]+(a2*varg2*a2))))
}
...
w=matrix(c(0,1),1,2,byrow=T)  # weight matrix

for (k in 1:ng) {
  corn=(Ntot[1,k]-1)/Ntot[1,k]            # adjust for unbiased estimator: exact fit in Mx
  for (i in 1:nvar) {                     # Mx divides by N rather than N-1
    for (j in 1:nfac) {
      omean1[k,i]=omean1[k,i]+L1[i,j]*(b1[k]*w[j]+lmean1[k,j])
    }}
...
for (k in 1:ng) {
  #  corn=(Ntot1-1)/Ntot1            # adjust for unbiased estimator: exact fit in Mx
  corn=(Ntot[2,k]-1)/Ntot[2,k]            # adjust for unbiased estimator: exact fit in Mx
  for (i in 1:nvar) {
    for (j in 1:nfac) {
      omean2[k,i]=omean2[k,i]+L2[i,j]*(b2[k]*w[j]+lmean2[k,j])
    }}
